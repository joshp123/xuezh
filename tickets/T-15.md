...
---
    ...
    ## Goal
    Provide a real pronunciation assessment backend via Azure Speech as the default for `audio.process-voice`.

    ## Deliverables
    - Backend id `azure.speech` for internal assessment
    - `audio.process-voice` defaults to Azure (no public backend override)
    ...
    ## Acceptance criteria
    - `audio.process-voice` uses Azure by default and returns required artifacts
      (normalized_input, transcript, assessment, feedback_voice_note)
