Feature: HSK audit reporting
  ...
  Scenario: Request an HSK compliance report
    ...
    When the client runs "xuezh dataset import --type hsk_vocab --path tests/fixtures/datasets/hsk_vocab_min.csv --json"
    Then the engine returns an OK envelope
    And the engine does not return recommendation fields

    When the client runs "xuezh report hsk --level 3 --window 30d --max-items 200 --max-bytes 200000 --json"
    Then the engine returns an OK envelope
