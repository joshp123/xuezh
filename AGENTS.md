...
- Persistent URL: `https://chinese.jjpcodes.com`. It is tailnet/private only: DNS points to `mini-server-1` and Caddy rejects non-private/non-tailnet clients.
- Durable runtime is NixOS-managed from `~/code/nix/nixos-config`: service `xuezh.service`, state dir `/var/lib/xuezh`, Linux service user `xuezh`, ingress in `hosts/nixos/mini-server-1/media-ingress/Caddyfile`.
- Do not deploy xuezh as a mutable Mac mini launchd checkout. The Mac mini is Josh's Mac/Codex/OpenClaw box, not the durable server host.
- Push is not deploy. If Josh says "deploy", "ship it", "make it live", or asks whether the phone URL is updated, the required end state is `mini-server-1` serving the new code. Update the `xuezh` flake input in `~/code/nix/nixos-config`, build/switch `#mini-server-1`, then verify the public URL.
...
- Copying production cram DB/audio to the server must preserve the workspace contents under `/var/lib/xuezh`. Verify `cram_items` has 689 HelloChinese and 461 Travel Survival rows after any data move.
- DNS/cert is handled by `hosts/nixos/mini-server-1/media-ingress/run-media-ingress.sh`; it UPSERTs `chinese.jjpcodes.com` and uses the existing Route53/lego setup. If DNS breaks, verify `dig +short chinese.jjpcodes.com`, `curl -skI https://chinese.jjpcodes.com`, and `journalctl -u media-ingress` on `mini-server-1`.
...
- Installed PWAs may boot from stale cached HTML that references an old `/assets/index-*.js` or `.css`. Vite must retain old assets where possible, and the Go web server must fall back missing entrypoint assets to the current entrypoint instead of 404ing. Verify with a known missing `/assets/index-oldhash.js` before claiming phone blank-page fixes are deployed.
